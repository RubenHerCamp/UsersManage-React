...
function App() {
  ...
  function resetUsers() {
    ...
    copy_users.current = [...init_users.current];
    setChange(!change);
  }

  function noOrderCountry() {
    setUsers([...copy_users.current]);
    setCountry(!country);
    setChange(!change);
  }
  ...
  function filterUserCountry(event) {
    ...
    const result = search ? copy_users.current.filter(user => user.location.country.toLowerCase().includes(search)) : copy_users.current;
    // console.log(result);
    setUsers(result);
    setChange(!change);
  }
